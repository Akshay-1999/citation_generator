...
async def write_converted_file_path(user_id : str , original_file : str , converted_file : str):
    """
    ...
    async with pool.acquire() as connection:
        try:
            logger.info(f"--- Writing converted file path: {converted_file} for file: {original_file} ---")
            await connection.execute(
                """
                INSERT INTO core.converted_resumes (user_id, original_file, converted_file_path , created_at , updated_at)
...
async def write_rejected_file_path(user_id : str , original_file : str , rejection_reason : str):
    ...
    async with pool.acquire() as connection:
        try:
            ...
            converted_path = await connection.fetchval(
                """
                UPDATE core.converted_resumes
                set rejection_reason = $1 , status = 'rejected' , is_deleted = true , deleted_at = now()
                WHERE user_id = $2 AND original_file = $3
                RETURNING converted_file_path
                """,
                rejection_reason,
                user_id,
                original_file
...
async def check_converted_status(user_id : str , file_path : str):
    ...
    async with pool.acquire() as connection:
        try:
            ...
            if row:
                logger.info(f"--- Converted file path found: {row['converted_file_path']} ---")
                return row["converted_file_path"]
            else:
